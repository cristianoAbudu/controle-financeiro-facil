      ...
      db.transaction(
        (tx) => {
          ...
          tx.executeSql("select * from categoria", [], (_, { rows }) => {
            // Certifique-se de que setCategoria é uma função válida
            setCategoria(rows);
          });
        },
        (e) => {
        ...
        // Certifique-se de que forceUpdate é uma função válida
        forceUpdate
      );
    })
      .then((mensagem) => {
      ...
      {Platform.OS === "web" ? (
        ...
        <>
          ...
          <ScrollView style={styles.listArea}>
            <Items key={`forceupdate-todo-${forceUpdateId}`} done={categoria} />
          </ScrollView>
        </>
      )}
    </View>
