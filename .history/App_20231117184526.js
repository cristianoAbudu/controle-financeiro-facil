...
function Items({ categoria, onPressItem }) {
...
export default function App() {
  ...
  const addDespesa = (text, valor, categoria) => {
    ...
    console.log(valor);
    if (valor === null || valor === "") {
      return Promise.reject("Valor é inválido");
    }
      ...
      {Platform.OS === "web" ? (
        ...
        <>
          <View>
            ...
            <Button
              title="Adicionar Despesa na Categoria"
              onPress={() => {
          ...
          <ScrollView style={{ flex: 1 }}>
            {categorias.map((categoria) => (
              <Items
                key={categoria.value} // Use a propriedade correta para a chave
                categoria={categoria.label} // Use a propriedade correta para a categoria
